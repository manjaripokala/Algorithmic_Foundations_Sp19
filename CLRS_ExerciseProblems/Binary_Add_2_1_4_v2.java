...
public class Binary_Add_2_1_4_v2{
	public static void main(String[] args){
		TwoArray A_and_B = accept_input();
		int[] C = output_C1(A_and_B.A, A_and_B.B);
		for(int i=0; i<C.length; i++){
			System.out.println("Elements of C1:" + C[i]);
		}
		int[] C2 = output_C2(A_and_B.A, A_and_B.B);
                for(int i=0; i<C2.length; i++){
			 System.out.println("Elements of C2:" + C2[i]);
 		}
		System.out.println(C.equals(C2));
	}
	
	public static TwoArray accept_input(){
		...
		return new TwoArray(A, B);
	}
	
	public static int[] output_C1(int[] A,int[] B){
	...
	public static int[] output_C2(int[] A,int[] B){
		int[] C = new int[A.length+1];
		int carry = 0;
		System.out.println("length of C:" + C.length);
		// we are only populating 2nd through last element, as carry will be moved to 0th
		for(int k=(A.length);k>=1; k--){
			C[k] = (A[k-1] + B[k-1] + carry) % 2;
			carry = (A[k-1] + B[k-1]) / 2; 
			//System.out.println("carry:" + k + carry);
		}
		// Always "carry" will be carried to 0th position
